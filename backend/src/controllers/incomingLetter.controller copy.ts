...
import fs from 'fs';
// PERBAIKAN: Impor logger dengan benar
...
const incomingLetterSchema = z.object({
  ...
  sender: z.string().min(2, 'Pengirim minimal 2 karakter').max(100, 'Pengirim maksimal 100 karakter'),
  recipient: z.string().min(2, 'Penerima minimal 2 karakter').max(100, 'Penerima maksimal 100 karakter'),
  processor: z.string().min(2, 'Pengolah minimal 2 karakter').max(100, 'Pengolah maksimal 100 karakter'),
...
// PERBAIKAN: Menambahkan kembali dispositionSchema yang hilang
...
export const getIncomingLetters = async (req: AuthenticatedRequest, res: Response): Promise<void> => {
  try {
    ...
    logger.error('Failed to get incoming letters:', error);
...
export const getIncomingLetterById = async (req: AuthenticatedRequest, res: Response): Promise<void> => {
  try {
    ...
    logger.error(`Failed to get letter ${req.params.id}:`, error);
    res.status(500).json({ error: 'Internal server error' });
  }
};
...
export const deleteIncomingLetter = async (req: AuthenticatedRequest, res: Response): Promise<void> => {
  try {
    ...
    logger.error(`Failed to delete letter ${req.params.id}:`, error);
    res.status(500).json({ error: 'Internal server error' });
  }
};

// =====================================
// Fungsi Controller Disposisi
// =====================================
...
const handleError = (error: unknown, res: Response) => {
  ...
  if (error instanceof Prisma.PrismaClientKnownRequestError && error.code === 'P2002') {
    res.status(409).json({ error: 'Data duplikat', details: error.meta?.target });
    return;
  }
